<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="服务域名/IP" prop="host">
        <el-input
...
const initFormData = () => {
  if (isEdit.value) {
    ...
    formData.host = rowData.value.host || ''
    formData.port = rowData.value.port|| ''
    formData.status = rowData.value.status || ''
  } else {
    // 新增：清空表单
    resetForm()
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 新增请求
    if (!isEdit.value) {
      res = await serverApi.add(formData)
    } else {
      // 编辑请求 - 传主键id
      const editData = {...formData}
      editData['id'] = rowData.value.id
      res = await serverApi.put(editData)
    }
    console.log('hub', res)
